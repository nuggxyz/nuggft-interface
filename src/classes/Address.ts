...
export class Address implements IAddress {
    ...
    public static ZERO = new Address(
        '0x0000000000000000000000000000000000000000',
    );

    public static NULL = new Address(
    ...
    public static shortenAddress(address: Address, chars = 4): string {
        try {
            return address.hash
                ? `${address.hash.substring(
                      0,
                      chars + 2,
                  )}...${address.hash.substring(42 - chars)}`
                : '';
        } catch (err) {
            return undefined;
        }
...
export class EnsAddress extends Address {
    private static ENS_NAME_REGEX =
        /^(([a-zA-Z0-9]+(-[a-zA-Z0-9]+)*\.)+)eth(\/.*)?$/;
