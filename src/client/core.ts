                ...
                immer__middleware((set, get) => {
                        ...
                        set((draft) => {
                            ...
                            if (!draft.epoch || epochId !== draft.epoch.id) {
                                draft.epoch = {
                                    ...
                                    endblock:
                                        web3.config.calculateStartBlock(epochId + 1, chainId) - 1,
                                    status: 'ACTIVE',
                                };
                                draft.nextEpoch = {
                                    id: epochId + 1,
                                    ...
                                    endblock:
                                        web3.config.calculateStartBlock(epochId + 2, chainId) - 1,
                                    status: 'PENDING',
                                };
                            }
                    ...
                    function updateProtocol(stateUpdate: ClientStateUpdate): void {
                        set((draft) => {
                            ...
                            if (stateUpdate.editingNugg)
                                draft.editingNugg = stateUpdate.editingNugg;
                            if (stateUpdate.recentSwaps)
                                draft.recentSwaps = stateUpdate.recentSwaps;
                            if (stateUpdate.recentItems)
                                draft.recentItems = stateUpdate.recentItems;
                            if (stateUpdate.activeSwaps)
                                draft.activeSwaps = stateUpdate.activeSwaps;
                            if (stateUpdate.activeItems)
                                draft.activeItems = stateUpdate.activeItems;
                    ...
                    function updateToken(tokenId: TokenId, data: LiveToken): void {
                        set((draft) => {
                            ...
                            if (
                                ...
                                JSON.stringify(get().liveTokens[tokenId])
                            ) {
