...
const Item: FC<{ item: LiveNuggItem; isOwner: boolean; nuggId: NuggId }> = ({
	item,
	isOwner,
	nuggId,
}) => {
	...
	return (
		<div style={styles.itemListItem}>
			<TokenViewer tokenId={item.tokenId} style={styles.listItemSvg} />
			<Label
				type="text"
				size="small"
...
const ItemList: FunctionComponent<Props> = ({ items, tokenId }) => {
	...
	return (
		...
		>
			{sortByField(sortByField([...items], 'feature', true), 'displayed', true).map(
				(x, i) => (
					<Item
						item={x}
